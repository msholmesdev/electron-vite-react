...
export class GameTableHandle {
  ...
  /**
   * Access to the `gameToken` unique index on the table `game`,
   * which allows point queries on the field of the same name
   * via the [`GameGameTokenUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.game.gameToken().find(...)`.
   *
   * Get a handle on the `gameToken` unique index on the table `game`.
   */
  gameToken = {
